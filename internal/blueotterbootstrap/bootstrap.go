...
import (
	...
	_ "github.com/libp2p/go-libp2p/p2p/protocol/circuitv2/relay"
)
...
// Starts up a libp2p node configured to act as a bootstrap for peers to connect to each other and a relay if direct connections through hole-punching fail.
// This bootstrap node advertises the same namespace to the larger network to allow new peers that don't know about its address to discover it and have
// a direct highway to discover other peers more quickly.
func StartBootstrapNode(ctx context.Context, port string) (host.Host, error) {
	...
	// Make the bootstrap node a relay so that it can act as a fallback if NAT hole-punching doesn't work
	...
	kDht, err := dht.New(ctx, host, dht.Mode(dht.ModeServer), dht.ProtocolPrefix("/ipfs"))
	if err != nil {
					...
					for p := range peerChan {
						...
						if host.Network().Connectedness(p.ID) != network.Connected {
							...
							if err := host.Connect(ctx, p); err != nil {
								...
								// retry connection if peer is still in list returned from FindPeers a minute after this attempt
